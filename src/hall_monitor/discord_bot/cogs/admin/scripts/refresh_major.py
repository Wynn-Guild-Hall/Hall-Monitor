"""``~script refresh_major`` — run the major-guild sweep on demand.
...
from hall_monitor.services import major_guilds, roster
...
async def main(ctx, *args: str) -> None:
    if major_guilds.is_refreshing():
        await ctx.reply(
            "a major-guild refresh is already running — this one would have "
            "produced the same numbers, so it wasn't started."
        )
        return

    message = await ctx.reply("major-guild refresh: loading leaderboards…")
    ...
    async def on_progress(done: int, total: int) -> None:
        ...
        if now - last_edit < _EDIT_INTERVAL_S and done != total:
            return
        last_edit = now
        await message.edit(content=f"major-guild refresh: {done}/{total} guilds…")

    summary = await major_guilds.refresh_all(on_progress=on_progress)
    if summary is None:
        # Lost a race with the scheduler between the check and the call.
        await message.edit(content="a major-guild refresh was already running.")
        return
    ...
    lines = [
        f"major-guild refresh done in {summary.seconds:.0f}s",
        f"- {summary.evaluated} guilds evaluated, {summary.major} major",
    ]
    if summary.failed:
        lines.append(
